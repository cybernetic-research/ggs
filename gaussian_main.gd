...
@onready var splatMultiMeshInstance = MultiMeshInstance3D.new()
...
func _load_ply_file():
	...
	splatMultiMesh.transform_format = MultiMesh.TRANSFORM_3D

	# Populate MultiMesh instances
	for i in range(num_vertex):
		var idx = i * num_properties
		var position = Vector3(vertices[idx], vertices[idx + 1], vertices[idx + 2])
		var scale = Vector3(vertices[idx + 3], vertices[idx + 4], vertices[idx + 5])
		var rotation_axis = Vector3(vertices[idx + 6], vertices[idx + 7], vertices[idx + 8]).normalized()
		var rotation_angle = vertices[idx + 9]
		var color = Color(
			clamp(vertices[idx + 10], 0.0, 1.0),
			clamp(vertices[idx + 11], 0.0, 1.0),
			clamp(vertices[idx + 12], 0.0, 1.0),
			clamp(vertices[idx + 13], 0.0, 1.0)
		)

		# Create transform
		var transform = Transform3D()
		transform.origin = position
		transform.basis = Basis().scaled(scale).rotated(rotation_axis, rotation_angle)

		# Set instance data
		splatMultiMesh.set_instance_transform(i, transform)
		splatMultiMesh.set_instance_color(i, color)
...
func _ready():
	# Initialize MultiMesh and add it to the scene
#	splatMultiMesh = MultiMesh.new()
	splatMultiMesh = $MultiMeshInstance3Dref.multimesh
	splatMultiMeshInstance.multimesh = splatMultiMesh
	#add_child(splatMultiMeshInstance)

	# Configure MultiMesh
	splatMultiMesh.instance_count = 0  # Start with no instances
	splatMultiMesh.transform_format = MultiMesh.TRANSFORM_3D  # Use 3D transforms
